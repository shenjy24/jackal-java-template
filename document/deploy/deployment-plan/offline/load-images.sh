...
IMAGE_DIR="$(resolve_deploy_path "${IMAGE_DIR:-../middleware-images}")"
MYSQL_IMAGE="${MYSQL_IMAGE:-mysql:8.4}"
MYSQL_IMAGE_TAR="$(resolve_deploy_path "${MYSQL_IMAGE_TAR:-$IMAGE_DIR/mysql-8.4.tar}")"
LOAD_RUNTIME_IMAGE="${LOAD_RUNTIME_IMAGE:-1}"
RUNTIME_IMAGE="${RUNTIME_IMAGE:-eclipse-temurin:21-jre}"
RUNTIME_IMAGE_TAR="$(resolve_deploy_path "${RUNTIME_IMAGE_TAR:-$IMAGE_DIR/eclipse-temurin-21-jre.tar}")"
LOAD_NGINX_IMAGE="${LOAD_NGINX_IMAGE:-0}"
NGINX_IMAGE="${NGINX_IMAGE:-nginx:1.30.2}"
NGINX_IMAGE_TAR="$(resolve_deploy_path "${NGINX_IMAGE_TAR:-$IMAGE_DIR/nginx-1.30.2.tar}")"
